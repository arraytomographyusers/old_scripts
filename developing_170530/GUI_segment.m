...
%             fprintf('Intensity Normalization: %d \n',norm)
            ...
            switch findobj(get(S.bg(1),'selectedobject'))
                case S.rd(1)
                    previsualitzation(path,name,ws,sf,'mean',min_size,max_size) 
            %        surface(path,name)
            %        close(S)
                case S.rd(2)
                    previsualitzation(path,name,ws,sf,'median',min_size,max_size) 
            %        surface(path,name)
            %        close(S)
                otherwise
                    set(S.ed,'string','None!') % Very unlikely I think.
...
%         end
    ...
    function [] = fm_call(varargin)     
         try
            ...
            Files = strcat(srcPath,'\*.tif');  % the folder in which ur images exists
            srcFiles = dir(Files);
