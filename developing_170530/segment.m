...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% Llegim el directori i tipus d'imatge %%%%%%%%%%%
srcFiles = strcat(srcPath,'\*.tif');  % the folder in which ur images exists
srcFiles = dir(srcFiles);
[x,y] = size(srcFiles);
    
mkdir(srcPath,'\segmented');

    %%%%%% Guardem parametres en un text %%%%%%
    fileID = fopen(strcat(srcPath,'\segmented\parameters.txt'),'w+');
...
for jj = 1 : x
    ...
    if and(exist(strcat(srcPath,'\segmented\',seq_name,'_segmentation.tif'))>0,check == 1)
                ...
                'Yes','No','Yes');
                    switch answ
                        case 'Yes'
                            ...
                            delete(strcat(srcPath,'\segmented\',seq_name,'_segmentation.tif'));
                        case 'No'
                ...
                'Yes','No','Yes');
                    switch answ
                        case 'Yes'
                            ...
                            delete(strcat(srcPath,'\segmented\*.tif'));
                        case 'No'
    ...
    I = read_stackTiff(strcat(srcPath,'\',srcFiles(jj).name)); % LLEGIM TOT L'STACK DE .TIF
    ...
    %% Extract connectivity and size information of each object
        ...
        [join2D, CC] = connectivity(bwImat,min,max);
    ...
    %% Save and print results
        ...
        for i=1:p
                outputFileName = strcat(srcPath,'\segmented\',seq_name,'_segmentation.tif');
